...
bool HTTPWeakClient::begin(String host, uint16_t port, String uri, bool https, String httpsFingerprint)
{
    if (https) {
...
bool HTTPWeakClient::begin(String host, uint16_t port, String uri, String httpsFingerprint)
{
    clear();
    _host = host;
    _port = port;
 ...
 * @param reuse bool
 */
void HTTPWeakClient::setReuse(bool reuse)
{
    _reuse = reuse;
}
 ...
 * @param timeout
 */
void HTTPWeakClient::useHTTP10(bool useHTTP10)
{
 ...
 * @return http code
 */
int HTTPWeakClient::PATCH(uint8_t * payload, size_t size) {
    return sendRequest("PATCH", payload, size);
}

int HTTPWeakClient::PATCH(String payload) {
 ...
 */
int HTTPWeakClient::sendRequest(const char * type, uint8_t * payload, size_t size)
{
    // connect to server
    if(!connect()) {
 ...
 * @return -1 if no info or > 0 when Content-Length is set by server
 */
int HTTPWeakClient::getSize(void)
{
    return _size;
}
 ...
 * @param first
 */
void HTTPWeakClient::addHeader(const String& name, const String& value, bool first, bool replace)
{
...
void HTTPWeakClient::collectHeaders(const char* headerKeys[], const size_t headerKeysCount)
{
...
bool HTTPWeakClient::connect(void)
{
    ...
    if (!_transportTraits) {
        DEBUG_HTTPCLIENT("[HTTP-Client] connect: HTTPWeakClient::begin was not called or returned error\n");
        return false;
    }
