...
public:
  ...
  void setReuseConnection(bool reuse) override { http_.setReuse(reuse); }
  ...
  void begin(const std::string &host, const std::string &path) override {
    http_.begin(host.c_str(), kFirebasePort, path.c_str(), kFirebaseCA);
  }
  ...
  int sendRequest(const std::string &method, const std::string &data) override {
    return http_.sendRequest(method.c_str(), (uint8_t *)data.c_str(),
                             data.length());
  }

  std::string getString() override { return http_.getString().c_str(); }

  Stream *getStreamPtr() override { return http_.getStreamPtr(); }
