...
String FirebaseArduino::push(const String &path, const JsonVariant &value) {
  ...
  value.printTo(buf);
  auto push =
      FirebasePush(host_, auth_, path.c_str(), buf.c_str(), http_.get());
  error_ = push.error();
...
bool FirebaseArduino::available() { return http_->getStreamPtr()->available(); }
...
bool FirebaseArduino::success() { return error_.code() == 0; }

bool FirebaseArduino::failed() { return error_.code() != 0; }

const String &FirebaseArduino::error() { return error_.message().c_str(); }
