...
public:
  ...
  virtual bool verify(WiFiClient &client, const char *host) { return true; }
};
...
void HTTPWeakClient::clear() {
  _returnCode = 0;
  _size = -1;
  _headers = "";
}

bool HTTPWeakClient::begin(String url, String httpsFingerprint) {
 ...
 * @param url String
 */
bool HTTPWeakClient::begin(String url) {
  _transportTraits.reset(nullptr);
  _port = 80;
...
bool HTTPWeakClient::beginInternal(String url, const char *expectedProtocol) {
...
bool HTTPWeakClient::begin(String host, uint16_t port, String uri) {
  clear();
  _host = host;
  _port = port;
  _uri = uri;
...
bool HTTPWeakClient::begin(String host, uint16_t port, String uri, bool https,
...
bool HTTPWeakClient::begin(String host, uint16_t port, String uri,
  ...
  _transportTraits = TransportTraitsPtr(new TLSTraits(httpsFingerprint));
  DEBUG_HTTPCLIENT(
      "[HTTP-Client][begin] host: %s port: %d url: %s httpsFingerprint: %s\n",
 ...
 */
void HTTPWeakClient::end(void) {
  if (connected()) {
    if (_tcp->available() > 0) {
      DEBUG_HTTPCLIENT(
          "[HTTP-Client][end] still data in buffer (%d), clean up.\n",
          _tcp->available());
 ...
 * @param reuse bool
 */
void HTTPWeakClient::setReuse(bool reuse) { _reuse = reuse; }

/**
 * set User Agent
 * @param userAgent const char *
 */
void HTTPWeakClient::setUserAgent(const String &userAgent) {
 ...
 */
void HTTPWeakClient::setAuthorization(const char *user, const char *password) {
  if (user && password) {
    String auth = user;
    auth += ":";
    auth += password;
 ...
 */
void HTTPWeakClient::setAuthorization(const char *auth) {
  if (auth) {
 ...
 */
void HTTPWeakClient::setTimeout(uint16_t timeout) {
  _tcpTimeout = timeout;
  if (connected()) {
 ...
 * @return http code
 */
int HTTPWeakClient::GET() { return sendRequest("GET"); }
 ...
 * @return http code
 */
int HTTPWeakClient::POST(uint8_t *payload, size_t size) {
  return sendRequest("POST", payload, size);
}

int HTTPWeakClient::POST(String payload) {
 ...
 * @return http code
 */
int HTTPWeakClient::PUT(uint8_t *payload, size_t size) {
  return sendRequest("PUT", payload, size);
}

int HTTPWeakClient::PUT(String payload) {
 ...
 * @return
 */
int HTTPWeakClient::sendRequest(const char *type, String payload) {
 ...
 */
int HTTPWeakClient::sendRequest(const char *type, uint8_t *payload,
                                size_t size) {
  // connect to server
  if (!connect()) {
 ...
 * @param size size_t           size for the message body if 0 not
 * Content-Length is send
 * @return -1 if no info or > 0 when Content-Length is set by server
 */
int HTTPWeakClient::sendRequest(const char *type, Stream *stream, size_t size) {
  ...
  if (buff) {
    ...
    while (connected() && (stream->available() > -1) &&
      ...
      if (sizeAvailable) {
        ...
        if (bytesWrite != bytesRead) {
          DEBUG_HTTPCLIENT("[HTTP-Client][sendRequest] short write, asked for "
                           "%d but got %d retry...\n",
          ...
          if (_tcp->getWriteError()) {
            DEBUG_HTTPCLIENT(
                "[HTTP-Client][sendRequest] stream write error %d\n",
                _tcp->getWriteError());
          ...
          bytesWrite =
              _tcp->write((const uint8_t *)(buff + bytesWrite), leftBytes);
          bytesWritten += bytesWrite;

          if (bytesWrite != leftBytes) {
            // failed again
            DEBUG_HTTPCLIENT("[HTTP-Client][sendRequest] short write, asked "
                             "for %d but got %d failed.\n",
    ...
    if (size && (int)size != bytesWritten) {
      DEBUG_HTTPCLIENT("[HTTP-Client][sendRequest] Stream payload bytesWritten "
      ...
      DEBUG_HTTPCLIENT(
          "[HTTP-Client][sendRequest] Stream payload written: %d\n",
          bytesWritten);
    }
 ...
 * @return -1 if no info or > 0 when Content-Length is set by server
 */
int HTTPWeakClient::getSize(void) { return _size; }

/**
 * returns the stream of the tcp connection
 * @return WiFiClient
 */
WiFiClient &HTTPWeakClient::getStream(void) {
  if (connected()) {
    return *_tcp;
  }
 ...
 * @return WiFiClient *
 */
WiFiClient *HTTPWeakClient::getStreamPtr(void) {
  if (connected()) {
    return _tcp.get();
  }
 ...
 */
int HTTPWeakClient::writeToStream(Stream *stream) {
 ...
 * @return String
 */
String HTTPWeakClient::getString(void) {
  StreamString sstring;

  if (_size) {
    // try to reserve needed memmory
    if (!sstring.reserve((_size + 1))) {
      DEBUG_HTTPCLIENT("[HTTP-Client][getString] not enough memory to reserve "
                       "a string! need: %d\n",
                       (_size + 1));
      return "";
    }
 ...
 * @return String
 */
String HTTPWeakClient::errorToString(int error) {
  switch (error) {
 ...
 * @param first
 */
void HTTPWeakClient::addHeader(const String &name, const String &value,
...
void HTTPWeakClient::collectHeaders(const char *headerKeys[],
...
String HTTPWeakClient::header(const char *name) {
  for (size_t i = 0; i < _headerKeysCount; ++i) {
...
String HTTPWeakClient::header(size_t i) {
...
String HTTPWeakClient::headerName(size_t i) {
  if (i < _headerKeysCount) {
...
int HTTPWeakClient::headers() { return _headerKeysCount; }

bool HTTPWeakClient::hasHeader(const char *name) {
  for (size_t i = 0; i < _headerKeysCount; ++i) {
    if ((_currentHeaders[i].key == name) &&
 ...
 */
bool HTTPWeakClient::connect(void) {
  ...
  if (!_transportTraits) {
    DEBUG_HTTPCLIENT(
        "[HTTP-Client] connect: HTTPWeakClient::begin was not called "
        "or returned error\n");
    return false;
  }
 ...
 * @return status
 */
bool HTTPWeakClient::sendHeader(const char *type) {
  if (!connected()) {
    return false;
  }
 ...
 */
int HTTPWeakClient::handleHeaderResponse() {
    ...
    if (len > 0) {
      ...
      if (headerLine == "") {
        ...
        if (_returnCode) {
          ...
          DEBUG_HTTPCLIENT("[HTTP-Client][handleHeaderResponse] Remote host is "
                           "not an HTTP Server!");
 ...
 */
int HTTPWeakClient::writeToStreamDataBlock(Stream *stream, int size) {
  int buff_size = HTTP_TCP_BUFFER_SIZE;
  int len = size;
  int bytesWritten = 0;
  ...
  if (buff) {
    ...
    while (connected() && (len > 0 || len == -1)) {
      ...
      if (sizeAvailable) {
        ...
        // are all Bytes a writen to stream ?
        if (bytesWrite != bytesRead) {
          DEBUG_HTTPCLIENT("[HTTP-Client][writeToStream] short write asked for "
                           "%d but got %d retry...\n",
                           bytesRead, bytesWrite);
          ...
          if (bytesWrite != leftBytes) {
            // failed again
            DEBUG_HTTPCLIENT("[HTTP-Client][writeToStream] short write asked "
                             "for %d but got %d failed.\n",
                             leftBytes, bytesWrite);
            free(buff);
    ...
    DEBUG_HTTPCLIENT("[HTTP-Client][writeToStreamDataBlock] connection closed "
                     "or file end (written: %d).\n",
                     bytesWritten);

    if ((size > 0) && (size != bytesWritten)) {
      DEBUG_HTTPCLIENT("[HTTP-Client][writeToStreamDataBlock] bytesWritten %d "
                       "and size %d mismatch!.\n",
                       bytesWritten, size);
      return HTTPC_ERROR_STREAM_WRITE;
    }
 ...
 * @return error
 */
int HTTPWeakClient::returnError(int error) {
  if (error < 0) {
